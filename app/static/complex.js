...
class ComplexRow extends React.Component {
	...
	render() {
		return <tr>
			<td className="mdl-data-table__cell--non-numeric">{this.props.complex.complex_id}</td>
			<td className="mdl-data-table__cell--non-numeric">{this.props.complex.name}</td>
		</tr>
	}
}
...
class ComplexPage extends React.Component {
	constructor(props) {
		super(props)
		this.state={pageItems:null, db_class:'Complex', db_col:'pk_id', asc: true}
	...
	buildUrl() {
		let request_url = (this.state.pageItems && this.state.pageItems.page) > 1 ? this.props.api_path+'/complex/'+page_num : this.props.api_path;
		...
		return <div className="demo-charts mdl-color--white mdl-shadow--2dp mdl-cell mdl-cell--12-col mdl-grid">
			{ this.state.pageItems ?
				...
				<table className="mdl-data-table mdl-js-data-table">
					<thead>
						<tr>
							<th className="mdl-data-table__cell--non-numeric">Complex Number</th>
							<th className="mdl-data-table__cell--non-numeric">Name</th>
						</tr>
					</thead>
					<tbody>
						{this.state.pageItems.complexes.map((complex, i) => {
							return <ComplexRow key={i} complex={complex} />
						})}
					</tbody>
				</table>
			</div>
...
ComplexPage.defaultProps = {api_path:'/api'};
