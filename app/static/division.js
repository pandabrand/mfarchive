/*** @jsx React.DOM */
class DivisionRow extends React.Component {
	constructor(props) {
		super(props)
	}
	...
	render() {
		return <tr>
			<td className="mdl-data-table__cell--non-numeric">{this.props.division.region_id}</td>
			<td className="mdl-data-table__cell--non-numeric">{this.props.division.name}</td>
		</tr>
	}
}
...
class DivisionPage extends React.Component {
	constructor(props) {
		super(props)
		this.state={pageItems:null, db_class:'Divison', db_col:'pk_id', asc: true}
		...
		return <div className="demo-charts mdl-color--white mdl-shadow--2dp mdl-cell mdl-cell--12-col mdl-grid">
			{ this.state.pageItems ?
				...
				<table className="mdl-data-table mdl-js-data-table">
					<thead>
						<tr>
							<th className="mdl-data-table__cell--non-numeric">Division Number</th>
							<th className="mdl-data-table__cell--non-numeric">Name</th>
						</tr>
					</thead>
					<tbody>
						{this.state.pageItems.divisions.map((division, i) => {
							return <DivisionRow key={i} division={division} />
						})}
					</tbody>
				</table>
			</div>
...
DivisionPage.defaultProps = {api_path:'/api'};
