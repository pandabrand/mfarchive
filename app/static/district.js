/*** @jsx React.DOM */
class DistrictRow extends React.Component {
	constructor(props) {
		super(props)
	}
	...
	render() {
		return <tr>
			<td className="mdl-data-table__cell--non-numeric">{this.props.district.area_number}</td>
			<td className="mdl-data-table__cell--non-numeric">{this.props.district.name}</td>
		</tr>
	}
}
...
class DistrictPage extends React.Component {
	constructor(props) {
		super(props)
		this.state={pageItems:null, db_class:'District', db_col:'pk_id', asc: true}
		...
		return <div className="demo-charts mdl-color--white mdl-shadow--2dp mdl-cell mdl-cell--12-col mdl-grid">
			{ this.state.pageItems ?
				...
				<table className="mdl-data-table mdl-js-data-table">
					<thead>
						<tr>
							<th className="mdl-data-table__cell--non-numeric">District Number</th>
							<th className="mdl-data-table__cell--non-numeric">Name</th>
						</tr>
					</thead>
					<tbody>
						{this.state.pageItems.districts.map((district, i) => {
							return <DistrictRow key={i} district={district} />
						})}
					</tbody>
				</table>
			</div>
...
DistrictPage.defaultProps = {api_path:'/api'};
